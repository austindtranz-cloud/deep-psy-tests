...
(function () {
  ...
  /* Map category IDs to Tilda URLs for multi-page mode */
  var CAT_URLS = {
    home: "/tests",
  ...
  function getCTA(test) {
    var isDataLoaded = (window.DEEP_TEST_DATA && window.DEEP_TEST_DATA[test.id]);
    var isRunnable = (test.isRunnable !== false) && (!!test.questions || isDataLoaded);
  ...
  function updateDashboardGrid() {
    ...
    // Detect if we are on a dedicated category page (via attribute)
    var container = document.querySelector(".deep-dashboard");
    var isDedicatedPage = container && container.getAttribute("data-category-id");

    var breadHtml = "";
    if (!isDedicatedPage) {
      breadHtml = '<nav class="deep-dash-breadcrumbs"><a href="' + CAT_URLS.home + '" data-nav="home">Все категории</a>';
    ...
    if (isAlpha) {
      ...
      Object.keys(reg).forEach(function(cId) {
        ...
        var catUrl = CAT_URLS[cId] || "#";
        
        cardsHtml += '<a href="' + catUrl + '" class="deep-tests-card deep-dash-cat-card" style="padding:0; overflow:hidden; cursor:pointer;" data-nav="' + cId + '">';
  ...
  window.deepRenderCatalog = function(containerId) {
    ...
    Object.keys(reg).forEach(function(cId) {
      ...
      navHtml += '<div class="deep-dash-nav-group">' +
        '<a class="deep-dash-nav-item" href="' + (CAT_URLS[cId] || "#") + '" data-nav="' + cId + '">' +
    ...
    container.addEventListener("click", function(e) {
      ...
      if (navItem) {
        ...
        // If it's a category transition, but NOT to the same page, we let it navigate normally (don't preventDefault)
